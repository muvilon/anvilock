...
static void handle_output_geometry(void *data, struct wl_output *wl_output, int32_t x, int32_t y,
    ...
    log_message(LOG_LEVEL_INFO, "Output: %s %s @ (%d, %d) [%d x %d] mm", make, model, x, y, physical_width, physical_height);
}

// This function will be called whenever output mode changes
static void handle_output_mode(void *data, struct wl_output *wl_output, uint32_t flags, int32_t width,
...
static void register_output(struct client_state *state, struct wl_registry *registry, uint32_t id, uint32_t version) {
