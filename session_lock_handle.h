...
void render_lock_screen(struct client_state *state) {
    ...
    wl_surface_damage(state->wl_surface, 0, 0, state->output_state.width, state->output_state.height); // Notify the compositor
...
static void create_lock_surface(struct client_state *state) {
    ...
    ext_session_lock_surface_v1_add_listener(
        state->ext_session_lock_surface_v1, &ext_session_lock_surface_v1_listener, state);
